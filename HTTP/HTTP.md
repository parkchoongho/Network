...
전 세계의 웹 브라우저, 서버, 웹 애플리케이션은 모두 HTTP(Hyper Text Transfer Protocol)를 통해 통신합니다. 수십억개의 이미지, HTML, 페이지, 텍스트 파일, 동영상 등등의 자료들이 쉴새없이 인터넷 사이사이를 이동하고 있습니다. HTTP는 송수신중 데이터를 파괴, 중복, 왜곡하는 것을 방지하기에 걱정하지 않아도 됩니다. 
