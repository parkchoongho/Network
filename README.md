...
유동 IP는 IP 주소가 계속 변하게 된다. 예를 들어, 현재 우리집 IP 주소가 198.78.108.86이라고 해보자. 그런데 우리 집이 한동안 집을 비우게 되어 공유기를 오랫동안 사용하지 않게 되면, 통신사가 우리 집에 부여한 IP 주소를 회수한다. 그 다음, 새로 통신사에 가입한 집에 그 IP 주소(198.78.108.86)를 부여하고 우리 집이 다시 돌아와 인터넷을 사용하면 다른 IP 주소를 부여한다. 이러한 이유로 인해, 가정에서는 서버를 운영하는 것이 좋지 못하다.(IP 주소가 계속 바뀌기 때문에) 

**Tip:** 서버를 운영하는 전용 서비스들의 경우에는 고정된 IP를 제공한다. 

이러한 유동IP로 인해 문제접이 발생하게 된다. 내가 oo2.org라는 도메인에 접속한다고 가정해보자. 그러면 내 컴퓨터는 Name Server로 가서 oo2.org에 해당하는 IP 값을 가져온다. 그리고 가져온 IP 주소를 바탕으로 접속을 시도하는데 유동 IP로 인해 oo2.org의 IP 주소 값이 바뀐것이다! 그런데 Name Server에는 이게 반영이 되지않았기에 이런 참사가 발생했다. 이러한 경우에는 어떻게 해야할까?

### DDNS (Dynamic DNS) 

위의 문제점을 해결하기 위해 나온것이 바로 DDNS이다. 위의 상황에서 원래의 IP 주소가 154.67.87.83이라고 해보자. 그런 상황에서 유동 IP로 인해 IP 주소가 154.67.87.84로 바뀌었다고 해보자. 그렇다면 이 IP 주소를 Name Server에 알려주어 해당 도메인의 IP 주소를 주기적으로 업데이트 해준다면 문제점은 해결된다. 이렇게 도메인과 호스트의 IP 주소를 지속적으로 일치화 즉, 동기화 시켜주는 DNS를 DDNS라고 한다.